...
description: Writing documents for agents. Use when creating, adapting, or editing skills, or modifying AGENTS.md or CLAUDE.md.
---
...
Apply the house vocabulary in [`LEXICON.md`](LEXICON.md) to every document you write or adapt: it names the canonical terms and the words to swap out. Agent documents have two readers, and a term must recruit both readers' priors to earn its place.
...
A leading word must recruit the _human_ reader's priors too: these documents are maintained, reviewed, and lived with by a person. A term that anchors the model but trips the maintainer is a bad trade; [`LEXICON.md`](LEXICON.md) records the replacements. Pick words from the maintainer's working vocabulary first.
...
- "a loop you believe in" → _red_, turning a fuzzy criterion into a binary observable state (the loop goes _red_ on the bug, or it doesn't).
...
[`writing-for-agents`](https://github.com/mattpocock/skills) skill (MIT),
with the house lexicon added.
